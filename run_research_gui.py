...
# 版本信息
CURRENT_VERSION = "0.1.1"  # 当前版本号
GITHUB_REPO_OWNER = "WindyJunsa"
...
class ResearchGUI:
    ...
    def create_online_api_tab(self, parent):
        ...
        # 初始化时批处理开关为False，所以模型选择框应该是disabled状态
        ...
        self.batch_model_combo = ttk.Combobox(batch_frame, textvariable=self.batch_model_var, 
                                               values=self.batch_supported_models, width=30, state=initial_batch_state)
...
请只返回JSON格式的响应，不要添加其他解释。"""
    ...
    def create_help_tab(self, parent):
        ...
        main_frame.pack(fill=tk.BOTH, expand=True)
        main_frame.columnconfigure(0, minsize=900)  # 左侧使用指南列最小宽度900像素
        main_frame.columnconfigure(1, weight=1)  # 右侧联系方式列可扩展
        ...
        left_guide_frame.columnconfigure(0, weight=0)  # 左列不扩展
        left_guide_frame.columnconfigure(1, weight=0)  # 右列不扩展
...
• 可随时点击"停止"按钮中断运行"""
        ...
        left_guide_text_widget.grid(row=0, column=0, sticky=(tk.W, tk.N, tk.S), padx=(0, 15))
...
• 用量监控基于实际请求时间计算，运行时间不足1分钟时按真实时间计算"""
        ...
        right_guide_text_widget.grid(row=0, column=1, sticky=(tk.W, tk.N, tk.S), padx=(15, 0))
...
"""
        ...
        update_title.pack(pady=(20, 15))
        
        update_text = """• 2026-01-04: 初始版本V0.1.0发布
• 加入硅基流动在线API支持
• 加入更新功能
• 加入批处理任务功能
• 加入模型用量监控
...
"""
        ...
        def check_in_thread():
            try:
                ...
                if has_update:
                    ...
                    # 已是最新版本
                    self.root.after(0, lambda: messagebox.showinfo("检查更新", f"当前已是最新版本！\n\n当前版本: v{current_version}", parent=self.root))
                    
            except requests.exceptions.RequestException as e:
                error_msg = str(e)
                self.root.after(0, lambda msg=error_msg: messagebox.showerror("检查更新失败", f"无法连接到GitHub检查更新：\n{msg}", parent=self.root))
            except Exception as e:
                error_msg = str(e)
                self.root.after(0, lambda msg=error_msg: messagebox.showerror("检查更新失败", f"检查更新时出错：\n{msg}", parent=self.root))
    ...
    def _download_update(self, release_data):
        """下载更新"""
        try:
            # 获取下载链接（精确查找PaperResearchTool.exe文件）
            ...
            for asset in assets:
                name = asset.get("name", "")
                # 精确匹配PaperResearchTool.exe
                if name == "PaperResearchTool.exe":
                    download_url = asset.get("browser_download_url", "")
                    filename = name
                    break
            
            if not download_url:
                messagebox.showwarning("下载更新", "未找到PaperResearchTool.exe文件", parent=self.root)
                return
                ...
                try:
                    ...
                    if auto_replace:
                        ...
                        if not hasattr(sys, '_MEIPASS'):
                            # 开发环境，提示手动替换
                            path = save_path
                            self.root.after(0, lambda p=path: messagebox.showinfo("下载完成", 
                                f"更新文件已下载到：\n{p}\n\n"
                        ...
                        # 手动安装模式
                        path = save_path
                        self.root.after(0, lambda p=path: messagebox.showinfo("下载完成", f"更新文件已下载到：\n{p}\n\n请手动安装更新。", parent=self.root))
                    
                except Exception as e:
                    progress_window.after(0, lambda: progress_window.destroy())
                    error_msg = str(e)
                    self.root.after(0, lambda msg=error_msg: messagebox.showerror("下载失败", f"下载更新文件时出错：\n{msg}", parent=self.root))
